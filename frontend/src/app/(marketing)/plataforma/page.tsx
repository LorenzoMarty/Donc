...
export default function PlatformPage() {
  return (
    <MarketingShell>
      <main className="marketing-container py-8 md:py-10">
        <AceternityGrid className="p-5 xs:p-6 md:p-10 lg:p-12">
          <div className="grid gap-10 md:grid-cols-[0.95fr_1.05fr] md:items-center">
            <Reveal>
              <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-primary/15 bg-primary/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.14em] text-[hsl(var(--green-700))]">
              ...
              <p className="marketing-copy mt-5 max-w-2xl text-lg">
            ...
            <Reveal delay={0.08}>
              <div className="grid gap-3">
                {pillars.map((pillar) => {
                  ...
                  return (
                    <div key={pillar.title} className="marketing-card flex gap-4 rounded-[14px] p-4">
                      <div className="marketing-icon h-10 w-10 shrink-0">
                        <Icon className="h-5 w-5" aria-hidden="true" />
                      </div>
                      <div>
                        <p className="font-semibold">{pillar.title}</p>
                        <p className="marketing-copy mt-1 text-sm">{pillar.text}</p>
                      </div>
                    </div>
                  );
                })}

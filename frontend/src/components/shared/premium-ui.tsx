...
export function PageHeader({
  ...
  return (
    <motion.div
      ...
      className="game-surface relative min-w-0 overflow-hidden bg-card p-4 xs:p-5 md:p-6"
    >
      ...
      <div className="relative flex flex-col justify-between gap-4 md:flex-row md:items-end md:gap-5">
        <div className="min-w-0">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              <h1 className="max-w-3xl text-[clamp(1.45rem,6vw,2.15rem)] font-semibold leading-tight tracking-normal md:text-[clamp(1.85rem,3vw,2.35rem)]">
                {title}
              </h1>
              {description && (
                <p className="mt-3 max-w-2xl text-sm font-medium leading-6 text-muted-foreground md:text-base">{description}</p>
              )}
            </div>
          </div>
        </div>
...
export function Surface({ children, className, delay = 0 }: { children: ReactNode; className?: string; delay?: number }) {
  ...
  return (
    <motion.section
      ...
      className={cn(
        "min-w-0",
        solidPrimary ? "game-surface bg-primary p-4 text-primary-foreground xs:p-5 md:p-6" : "game-surface bg-card p-3 xs:p-4 md:p-5",
        className,
      )}
    >
      {children}

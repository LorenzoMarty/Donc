...
} from "@/components/app/achievements-system";
...
import { apiFetch, type EssayHistory, type Exercise } from "@/lib/api";
  ...
  return (
    <div className="space-y-5 md:space-y-6">
      <PageHeader
        ...
        title="Platine sua evolucao"
        description="Uma colecao de trofeus para registrar constancia, dominio de redacao, progresso nas trilhas e desafios secretos."
        action={
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Surface>
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">Nivel {user.level}, {user.xp} XP e {user.streak_days} dias de sequencia.</p>
        </Surface>
        <Surface>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase text-muted-foreground">Redacao</p>
          ...
          <p className="mt-3 text-sm leading-6 text-muted-foreground">Melhor nota registrada e {metrics.essaysWritten} textos no historico.</p>
        </Surface>
        <Surface>
          <div className="flex items-center justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase text-muted-foreground">Laboratorio</p>
              <h2 className="mt-1 text-xl font-black tracking-normal">{connectiveWins + puzzleWins + argumentWins} vitorias</h2>
            </div>
            <Gem className="h-5 w-5 text-secondary" aria-hidden="true" />
          </div>
          <p className="mt-3 text-sm leading-6 text-muted-foreground">Conectivos, puzzles e argumentacao alimentam trofeus de dominio.</p>
        </Surface>
      </div>
      ...
      <XPRewardModal
        ...
        actionLabel="Adicionar a colecao"
        onClose={closeReward}
      />
    </div>
  );

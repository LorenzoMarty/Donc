...
const trackBlueprints = [
  {
    ...
    subtitle: "Campanha linear para dominar leitura, inferencia e distratores do ENEM.",
    titleReward: "Mestre da Interpretacao",
    difficulty: "Base ao avancado",
    color: "primary" as const,
    match: ["Compreensao", "Textos"],
  },
  {
    id: "gramatica",
    title: "Gramatica Aplicada",
    subtitle: "Pontuacao, coesao e norma-padrao em uma progressao controlada.",
    titleReward: "Aprendiz da Gramatica",
    difficulty: "Precisao",
    color: "accent" as const,
    match: ["Pontuacao", "Coesao", "Funcoes"],
  },
  {
    id: "redacao",
    title: "Redacao ENEM",
    subtitle: "Competencias, tese, repertorio e intervencao em etapas curtas.",
    titleReward: "Rei da Argumentacao",
    difficulty: "Desafio ENEM",
    color: "gold" as const,
    match: ["Competencia", "Redacao", "Compreensao"],
  },
];
...
const phaseTemplates = [
  {
    ...
    difficulty: "Base",
    nodes: [
      { label: "Exercicio 1", kind: "exercise" as const },
      { label: "Exercicio 2", kind: "exercise" as const },
      { label: "Marco 1", kind: "checkpoint" as const },
      { label: "Revisao rapida", kind: "review" as const },
  ...
  },
  {
    id: "fase-2",
    title: "Fase 2",
    subtitle: "Etapa intermediaria",
    difficulty: "Intermediaria",
    nodes: [
      { label: "Exercicio 6", kind: "exercise" as const },
      { label: "Exercicio 7", kind: "exercise" as const },
      { label: "Marco 2", kind: "checkpoint" as const },
      { label: "Revisao guiada", kind: "review" as const },
      { label: "Desafio relampago", kind: "challenge" as const },
    ],
  },
  {
    id: "boss",
              ...
              <div key={node.id}>
                {phaseStarts && (
                  <div className="relative z-10 mb-2 ml-20 md:ml-0 md:text-center">
                    <span className="inline-flex items-center gap-2 rounded-md border bg-background/80 px-3 py-1 text-xs font-black uppercase text-muted-foreground backdrop-blur">
  ...
  return (
    <div className="pointer-events-none absolute bottom-10 left-[46px] top-10 w-1 overflow-hidden rounded-full bg-border/70 md:left-1/2 md:-translate-x-1/2">
      <motion.div
        ...
        className="w-full rounded-full bg-gradient-to-b from-accent via-secondary to-primary shadow-glow"
      />
    </div>
  );
}
...
export function ExerciseNode({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "group relative grid h-20 w-20 place-items-center rounded-2xl border border-secondary/55 bg-gradient-to-br from-secondary to-[#e6c65d] text-secondary-foreground shadow-glow",
        active && "ring-4 ring-secondary/25",
      )}
      aria-label={`Abrir ${node.label}`}
    >
      <span className="absolute inset-1 rounded-xl border border-white/35 bg-white/12" />
      <span className="relative grid h-11 w-11 place-items-center rounded-xl bg-white/20 shadow-sm">
...
export function CompletedNode({
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.97 }}
      className={cn(
        "relative grid h-20 w-20 place-items-center rounded-2xl border border-accent/42 bg-gradient-to-br from-accent to-[#78c6ad] text-accent-foreground shadow-sm",
        active && "ring-4 ring-accent/20",
      )}
      aria-label={`Rever ${node.label}`}
    >
      <span className="absolute inset-1 rounded-xl border border-white/30 bg-white/12" />
      <motion.span
        initial={{ scale: 0.7, rotate: -12 }}
        animate={{ scale: 1, rotate: 0 }}
        className="relative grid h-11 w-11 place-items-center rounded-xl bg-white/18"
      >
...
export function BossNode({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "relative grid h-24 w-24 place-items-center rounded-[1.35rem] border text-secondary-foreground shadow-premium",
        node.state === "locked" && "border-white/8 bg-ink/22 text-muted-foreground opacity-80 dark:bg-black/36",
        node.state === "available" && "border-secondary/60 bg-gradient-to-br from-secondary via-[#dfbd4d] to-primary shadow-glow",
        completed && "border-accent/40 bg-gradient-to-br from-accent to-secondary",
        active && "ring-4 ring-secondary/25",
      )}
      aria-label={`${node.label} etapa final`}
    >
      <span className="absolute inset-1 rounded-2xl border border-white/25 bg-white/10" />
      <span className="relative grid h-14 w-14 place-items-center rounded-2xl bg-white/16">
...
export function XpBar({
  ...
  return (
    <div className="glass-surface overflow-hidden rounded-lg p-4">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase text-muted-foreground">Progressao</p>
          <p className="mt-1 text-2xl font-black tracking-normal">Nivel {level}</p>
        </div>
    ...
    return (
      <section className="glass-surface grid min-h-[420px] place-items-center rounded-lg p-6 text-center">
        <div>
          ...
          <p className="mt-2 max-w-sm text-sm leading-6 text-muted-foreground">A etapa dourada e a unica liberada agora. Complete para abrir a proxima.</p>
        </div>
      </section>
    );
  }
      ...
      <div className="space-y-4 p-4 md:p-5">
        ...
        {result ? (
          <div className={cn("rounded-lg border p-4 text-sm leading-6", result.is_correct ? "bg-accent/10" : "bg-destructive/10")}>
            <p className="font-black">{result.is_correct ? `Etapa concluida +${result.xp_earned} XP` : "Etapa nao concluida"}</p>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <h2 className="mt-1 text-xl font-black tracking-normal">Interpretacao Textual</h2>
        </div>
...
function templateSubtitle(kind: ExerciseNodeKind, exercise?: Exercise) {
  ...
  if (kind === "review") return `Revisao curta antes da proxima fase.`;
  if (kind === "challenge") return `Desafio rapido com ganho maior de XP.`;
  return exercise?.skill ?? "Exercicio bloqueado";
}

...
from src.schemas.lessons import CourseRead, LessonProgressRead, LessonProgressUpdate, LessonRead
...
def courses(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)) -> ApiResponse[list[CourseRead]]:
    return success_response(LessonService(db).list_courses(current_user.id))
